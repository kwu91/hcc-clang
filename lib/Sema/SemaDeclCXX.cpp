...
#endif
  for (/*auto*/Type *it : ClassDecl->getASTContext().types()) {
    if(/*(**/it/*)*/->isRecordType()) {
...
void Sema::DeclareAMPTrampolineName(CXXRecordDecl *ClassDecl,
  DeclarationName Name) {
  SourceLocation CurrentLocation = ClassDecl->getLocation();
...
void CreateDummyAMPTrampoline(Sema& S, DeclarationName Name,
                                                                     CXXRecordDecl *&ClassDecl, CXXMethodDecl *&Trampoline) {
  if(Trampoline)
    return;
  SourceLocation CurrentLocation = ClassDecl->getLocation();
  DeclarationNameInfo DNI(Name, CurrentLocation);
